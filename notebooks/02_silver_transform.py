...
latest = Window.partitionBy("order_id").orderBy(
    F.col("source_updated_at").desc_nulls_last(), F.col("_ingested_at").desc()
)
...
checked = deduped.withColumn("quality_reason", quality_reason).withColumn("_silver_processed_at", F.current_timestamp())
